...
def _payload(project_id, created_by, **overrides) -> BugCreate:
...
class TestBugService:
    ...
    def test_bug_number_auto_increments_per_project(self, db_session):
        ...
        user = _make_user(db_session)
        project = _make_project(db_session, user=user)
        first = service.create(db_session, _payload(project.id, user.id))
        second = service.create(db_session, _payload(project.id, user.id))
        third = service.create(db_session, _payload(project.id, user.id))
    ...
    def test_bug_number_resets_per_project(self, db_session):
        ...
        b1 = service.create(db_session, _payload(p1.id, user.id))
        b2 = service.create(db_session, _payload(p2.id, user.id))
        b1b = service.create(db_session, _payload(p1.id, user.id))
    ...
    def test_update_partial(self, db_session):
        ...
        created = service.create(
            db_session,
            _payload(project.id, user.id, title="keep me"),
        )
        updated = service.update(
            db_session,
            created.id,
    ...
    def test_list_filter_by_project(self, db_session):
        ...
        p1 = _make_project(db_session, user=user)
        p2 = _make_project(db_session, user=user)
        in_p1 = service.create(db_session, _payload(p1.id, user.id))
    ...
    def test_list_filter_by_status(self, db_session):
        ...
        user = _make_user(db_session)
        project = _make_project(db_session, user=user)
        service.create(db_session, _payload(project.id, user.id))  # status='new'
        accepted = service.create(db_session, _payload(project.id, user.id, status="accepted"))
    ...
    def test_list_filter_by_severity(self, db_session):
        ...
        user = _make_user(db_session)
        project = _make_project(db_session, user=user)
        service.create(db_session, _payload(project.id, user.id, severity="major"))
        critical = service.create(db_session, _payload(project.id, user.id, severity="critical"))
    ...
    def test_list_filter_by_source(self, db_session):
        ...
        user = _make_user(db_session)
        project = _make_project(db_session, user=user)
        service.create(db_session, _payload(project.id, user.id))  # internal
        customer = service.create(db_session, _payload(project.id, user.id, source="customer"))
    ...
    def test_list_filter_by_created_by(self, db_session):
        ...
        a_bug = service.create(db_session, _payload(project.id, owner_a.id))
        service.create(db_session, _payload(project.id, owner_b.id))
