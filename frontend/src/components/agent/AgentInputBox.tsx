// AgentInputBox — the relay input box on the AI Agent tab (CR-V2-022 / CR-V2-015, design §4.4.1).
//
// The Manažér's message is RELAYED through the engine (POST /pipeline/{version}/relay) as the AI Agent's next
// `--resume` turn — SPIKE-IO Model B: the engine is the SOLE writer to the warm `claude` session; this is
// NEVER a raw keystroke into the PTY. When a turn is in flight the relay returns `deferred: true` (the
// message is enqueued behind the in-flight turn and lands at the next turn boundary) — we surface the
// design-mandated hint "engine práve pracuje, správa sa pošle po dokončení ťahu". The same hint covers a
// raw-PTY `write_rejected` frame (the single-writer guard) routed in from the parent.
...
interface Props {
  ...
  /** A raw-PTY single-writer rejection bubbled up from the WS (CR-V2-015) — shown as the same busy hint. */
  writeRejected?: string | null;
  /** Clear the bubbled-up write_rejected signal once the hint has shown. */
  onClearWriteRejected?: () => void;
  disabled?: boolean;
}

export function AgentInputBox({ onRelay, writeRejected, onClearWriteRejected, disabled }: Props) {
  const [text, setText] = useState("");
  const [sending, setSending] = useState(false);
  const [hint, setHint] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  // A raw-PTY write_rejected frame → the same busy hint (steer the Manažér to the relay, which is THIS box).
  const effectiveHint = hint ?? (writeRejected ? ENGINE_BUSY_HINT : null);
