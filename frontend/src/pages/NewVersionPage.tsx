...
export default function NewVersionPage() {
  ...
  async function refreshGitStatus(): Promise<GitStatus | null> {
    ...
    try {
      ...
      setGitStatus(gs);
      setGitError(null); // a successful read CLEARS the block — otherwise nothing outside DirtyTreeGuard could
      return gs;
    } catch (err: unknown) {
        ...
        <div className="max-w-xl mx-auto">
          ...
          {!savedVersion && (isDirty || gitError) && (
            <div className="mb-5 space-y-2">
              ...
              {gitError && (
                <div className="rounded-lg bg-[var(--color-state-error-bg)] border border-[var(--color-state-error-bg)] p-3">
                  <ErrorNote error={gitError} />
                  {/* The way out. `treeUnverified` disables "Uložiť Zadanie", and the only controls that
                      cleared it lived in DirtyTreeGuard — which renders solely for a DIRTY tree, a state a
                      failed check can never reach (the status stays null). So a single failed preflight
                      disabled founding for good, greyed and unexplained, with nothing on screen to press. */}
                  <button
                    type="button"
                    onClick={() => void refreshGitStatus()}
                    disabled={gitBusy}
                    className="mt-2 rounded-lg border border-[var(--color-border-default)] px-2.5 py-1 text-xs text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)] disabled:opacity-50"
                  >
                    Skúsiť znova
                  </button>
                </div>
            ...
            {!savedVersion ? (
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button
                  ...
                  title={
                    treeUnverified
                      ? "Nepodarilo sa overiť, či projekt nemá neuložené zmeny — skús to znova."
                      : isDirty
                        ? "Projekt má neuložené zmeny — najprv ich ulož alebo zahoď."
                        : undefined
                  }
