...
> Convention: backend CRs verified with `ruff format --check && ruff check && pytest` + the named drift/contract test; FE CRs with `npm run type-check && npm run build`. No mypy (NEX Studio backend has no Python type-checker by decision). **Milestone-boundary exception:** a CR that lands an enum/CHECK *value* rename ahead of its deferred engine writers (e.g. CR-V2-004, whose writers are re-pointed in CR-V2-009) cannot show per-CR `pytest`-green against the live engine — full `pytest` + `alembic upgrade head` for that CR run at the Milestone boundary once the writers land; the per-CR gate is then the DB-free set (ruff + `py_compile` + `alembic heads` single-head + grep-clean + FE `type-check`). Such CRs name this exception explicitly in their Verification line.
...
- Verification (DB-free, this CR): `ruff format --check && ruff check` + `py_compile` clean on the changed `.py`; `alembic heads` shows the single new head 071 chained after 070; `grep -rin "director" backend/services frontend/src` returns only intentional historical/rationale references (Coordinator-relay design prose deferred to CR-V2-009, NOT live operator labels); FE `npm run type-check` clean and renders "čaká na Manažéra" / "Na rade: Manažér" (no live label still reads "Director"); no column-rename appears in migration 071 (only the CHECK + data-migrate). **Full `pytest` + `alembic upgrade head` run at the Milestone-A boundary, NOT per-CR here:** this CR lands the STATUS *value* + participant-token rename in the model + migration 071 CHECKs, but the ~65 engine writers in `orchestrator.py`/`pipeline_runner.py` that still emit the OLD `awaiting_director`/`director` values are deliberately deferred to CR-V2-009/CR-V2-006/CR-V2-029 (see scope notes above + R-LISTENERS). Until those writers are re-pointed, a write of an old value would be rejected by the new CHECK, so a green full pytest against the live engine is only meaningful once CR-V2-009 lands — it is asserted at the milestone boundary, not at this CR. This is the documented exception to the §3 per-CR `pytest`-green convention (line 141), forced by the spec's own writer-deferral ordering.
