...
- **⚠ OQ-2-GATED (pre-Milestone-B blocker).** This CR commits to keeping `invoke_claude` verbatim for the AI Agent and to ONE warm PTY the engine drives. That presupposes OQ-2's answer (warm interactive PTY vs headless `-p` observe-only). **OQ-2 must be resolved before Milestone B starts**, fed by the **SPIKE-IO gating spike** (the IO/locking model carved out of CR-V2-015, run up-front — see CR-V2-015 and §6 R-IO). If OQ-2 resolves "headless", CR-V2-007's premise (warm PTY the engine writes to) changes and this CR + its inheritors (CR-010/012/015/016/022) are re-scoped. Do NOT build CR-V2-007 until SPIKE-IO has fixed OQ-2.
...
- **⚠ OQ-1-contingent (`cr`/`bug` flows).** Today `_stage_order_for` (`orchestrator.py:6261-6262`) returns the full `STAGE_ORDER` for `cr`/`bug` exactly as for `new_version`. If OQ-1 keeps `cr`/`bug` as full-pipeline flows, the 4-phase `STAGE_ORDER` serves them unchanged (same path); if OQ-1 drops them (plan default), CR-V2-031 removes the values. Either way **no separate stage-order variant is needed** — `cr`/`bug` reuse the 4-phase path or are removed; this CR does NOT add a distinct multi-path. (Resolve OQ-1 before CR-V2-031; this CR is buildable under either answer.)
...
**SPIKE-IO (pre-Milestone-B gating spike — resolves OQ-2 / R-IO, feeds CR-V2-007 & CR-V2-015)**
- **This is a time-boxed design+prototype spike, run BEFORE Milestone B**, not a shippable CR. It answers OQ-2 (warm interactive `claude` PTY the engine writes to, vs headless `-p` the human only observes) and designs the single coherent IO/locking model for the case where the engine drives the AI Agent PTY AND the Manažér types into it concurrently (R-IO — the biggest unknown). Output: a decided IO/locking model (write-arbitration / turn-token) + the chosen execution substrate, recorded in the decision-log.
...
- **Store decision (OQ-4) — DEFAULT = per-project `MEMORY.md`** the charter rules tell the agent to read at session start and write to (Dedo-style; no schema, no migration). This is the recommended default (zero schema, human-readable, mirrors the proven Dedo pattern). **Only if OQ-4 resolves "DB store"** does this CR add migration **077** (a per-project memory table) — otherwise no migration.
...
- Files: new memory module + path convention; `backend/services/{knowledge_*,live_documents}.py` (reused/folded); AI Agent charter (KB+memory rules); migration **077** ONLY if OQ-4=DB.
...
**CR-V2-028 — Fast-fix lane simplified (AI Agent short path + light Auditor) ⚠ OQ-3-BLOCKED**
- **⚠ OQ-3-BLOCKED — do NOT build before OQ-3 is resolved (genuine design tension, R-FF).** Today's fast-fix ENDS in `_fast_fix_auto_deploy`; design D6 (§3.1) makes deploy ALWAYS manual + per-customer + outside the dial. These contradict: a v2 fast-fix cannot auto-deploy to a customer instance if every deploy is a manual per-customer UAT/PROD click. This must be resolved by the Manažér before Milestone H builds CR-V2-028 (and its dependents) — escalate, do not silently pick a side. **Plan default pending OQ-3:** fast-fix stops at "verified" (same `Hotovo`/verified boundary as a normal version) and the patch goes through the ordinary manual per-customer UAT/PROD click; the legacy `_fast_fix_auto_deploy` auto-deploy is retired (consistent with D6 and with CR-V2-014's "Hotovo ≠ deployed"). The faster-path value is in skipping heavy Návrh + the per-task Auditor, NOT in auto-deploy.
...
**CR-V2-031 — Remove `cr`/`bug` flow_type (pending OQ-1)**
- Scope: if confirmed (OQ-1), drop `cr`/`bug` from `FLOW_TYPE_VALUES` + CHECK (migration 075); only `new_version` + `fast_fix` survive. (Hold until Manažér confirms no CR/bug full-pipeline flow is wanted.)
...
| 077 | per-project memory table — **CONDITIONAL** | ONLY emitted if OQ-4 resolves "DB memory store"; the default (per-project `MEMORY.md`) needs no migration. (CR-V2-016.) |
...
- **R-FF:** today's fast-fix ENDS in `_fast_fix_auto_deploy`; D6 makes deploy always manual + per-customer — a hard contradiction. **CR-V2-028 is OQ-3-BLOCKED**; the plan default (pending Manažér confirm) is to stop fast-fix at "verified" and retire the in-lane auto-deploy, sending the patch through the normal manual per-customer Nasadiť (consistent with D6 and CR-V2-014's "Hotovo ≠ deployed"). A genuine design tension to escalate, not silently resolve.
...
1. **OQ-1 (cr/bug flows):** Do any `cr`/`bug` full-pipeline flows survive in v2, or are all non-version changes funneled through the fast-fix lane? (Gates CR-V2-031 + `FLOW_TYPE_VALUES`.) Plan assumes drop unless confirmed.
2. **OQ-2 (warm-context execution model) — PROMOTED TO PRE-MILESTONE-B BLOCKER:** Is the AI Agent ONE long-lived interactive `claude` PTY process the engine prompts by writing to its stdin (the navrh's "same kind the Director uses with Dedo"), or a headless `-p` process the human only observes? This decides whether `invoke_claude` survives for the AI Agent or only for the Auditor + helpers. **Resolved by SPIKE-IO before Milestone B**; gates CR-V2-007 (and thus 010/012/015/016/022). (R-IO.)
3. **OQ-3 (fast-fix auto-deploy vs D6) — BLOCKS CR-V2-028:** Where does fast-fix auto-deploy land given deploy-is-manual-per-customer? Does it auto-deploy to a default customer, or stop at "verified" and require the manual per-customer UAT/PROD click like every other version? **CR-V2-028 is marked OQ-3-BLOCKED; plan default = stop at verified, retire `_fast_fix_auto_deploy`, normal manual per-customer Nasadiť.** Must be confirmed before Milestone H.
4. **OQ-4 (AI-Agent memory store):** A per-project `MEMORY.md` the rules tell the agent to read/write (Dedo-style — **the plan default, no migration**), or a DB store (→ migration 077)? Decides the memory-injection mechanism. (Drives CR-V2-016.)
5. **OQ-5 (deploy boundary / build order):** Can the in-pipeline Verifikácia be built first with deploy stubbed (CR-V2-032 self-deploy deferred to cutover), and does the deploy backend own per-customer secrets directly or point into the existing `credentials.py` store? (Affects CR-V2-014/025/026 sequencing.)
6. **OQ-6 (cutover data strategy):** For historical v1 `pipeline_state`/`message` rows at cutover — migrate stage/actor values to the nearest v2 equivalent, or leave v1 versions read-only under a legacy enum? (Drives migration 069 strategy.)
7. **OQ-7 (route naming):** Do `/cockpit`→`/vyvoj` and `/coordinator`→`/ai-agent` URLs change, or stay with labels-only renames (affects bookmarks, PersistentTerminalsLayer keying)? Are Zákazníci/UAT/PROD nested `/projects/:slug/*` or global routes reading the pin?
8. **OQ-8 (task-plan persistence scope):** localStorage (per-browser) vs backend per-user pref (follows the Manažér across devices) for expand/collapse state? (Drives CR-V2-023.)
9. **OQ-9 (Auditor effort + dial coupling):** Does the dial raise the Auditor's `--effort` when autonomy is high (intensity scales)? Couples the effort resolver to the dial, which it does not touch today. (Drives CR-V2-007/008/013.)
10. **OQ-10 (status-block survival):** Does v2 keep a machine-readable `<<<PIPELINE_STATUS>>>` block at all, or does the engine infer phase/await from the live PTY session state? (Drives CR-V2-006 keep-vs-replace depth.)
11. **OQ-11 (Web-archetype commerce add-on) — DEFERRED:** The Web archetype's optional eshop/commerce capability (cart / checkout / payments + **bidirectional IS-integration**, the latter being the real complexity per design §4.2) is **deferred out of v2.0.0**. v0.x Web = managed/monitored site WITHOUT commerce; CR-V2-005 scaffolds the two surfaces (admin-FE + public-site) and leaves a documented extension seam but emits NO commerce code. When the first real eshop project arrives, the commerce add-on gets its own design round (like Mobil, design §8 Open #1). Confirm this deferral is intended, or pull the add-on scaffold into CR-V2-005's scope.
